...
namespace UnityStandardAssets.Water
{
  ...
  public class PlanarReflection : MonoBehaviour
  {
    ...
    void OnPreCull()
    {
      ...
      if (!m_HelperCameras.ContainsKey(cam))
      {
        m_HelperCameras.Add(cam, false);
      }

      if (!m_HelperCameras[cam])
      {
        if (!m_ReflectionCamera)
        {
          m_ReflectionCamera = CreateReflectionCameraFor(cam);
        }

        RenderReflectionFor(cam, m_ReflectionCamera);
        m_HelperCameras[cam] = true;
      }
    }
